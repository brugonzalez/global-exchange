...
class Command(BaseCommand):
    help = 'Inicializa el sistema Global Exchange con datos básicos'

    def handle(self, *args, **options):
        self.stdout.write('Inicializando el sistema Global Exchange...')
        ...
        datos_plantillas = [
            ...
            },
            {
                ...
                Se ha creado una nueva transacción en su cuenta:
        ...
        datos_usuarios_admin = [
            {
                'username': 'admin1',
                'email': 'admin1@globalexchange.com',
                'password': 'admin1',
                'nombre_completo': 'Administrador 1'
            },
            {
                'username': 'admin2',
                'email': 'admin2@globalexchange.com',
                'password': 'admin2',
                'nombre_completo': 'Administrador 2'
            }
        ]
        
        for dato_admin in datos_usuarios_admin:
            if not Usuario.objects.filter(username=dato_admin['username']).exists():
                superusuario = Usuario.objects.create_user(
                    username=dato_admin['username'],
                    email=dato_admin['email'],
                    password=dato_admin['password'],
                    nombre_completo=dato_admin['nombre_completo'],
                    is_staff=True,
                    is_superuser=True,
                    email_verificado=True
                )
                self.stdout.write(f'✓ Superusuario creado: {superusuario.username}')
                self.stdout.write(self.style.WARNING(f'Contraseña por defecto: {dato_admin["password"]} (¡cambiar inmediatamente!)'))
        
        # Crear usuarios regulares
        datos_usuarios_regulares = [
            {
                'username': f'usuario{i}',
                'email': f'usuario{i}@globalexchange.com',
                'password': f'usuario{i}',
                'nombre_completo': f'Usuario {i}'
            }
            for i in range(1, 7)  # Crea desde usuario1 hasta usuario6
        ]
        
        for dato_usuario in datos_usuarios_regulares:
            if not Usuario.objects.filter(username=dato_usuario['username']).exists():
                usuario = Usuario.objects.create_user(
                    username=dato_usuario['username'],
                    email=dato_usuario['email'],
                    password=dato_usuario['password'],
                    nombre_completo=dato_usuario['nombre_completo'],
                    is_staff=False,
                    is_superuser=False,
                    email_verificado=True
                )
                self.stdout.write(f'✓ Usuario creado: {usuario.username}')
                self.stdout.write(self.style.WARNING(f'Contraseña por defecto: {dato_usuario["password"]} (¡cambiar inmediatamente!)'))
        ...
        self.stdout.write('4. Inicie sesión con usuarios administradores: admin1/admin1 o admin2/admin2')
        self.stdout.write('5. O inicie sesión con usuarios regulares: usuario1/usuario1, usuario2/usuario2, etc.')